...
import { loadSavingsGoals } from '../data/savingsStorage';
...
export default function ProductSettings({ transactions, dark, onTheme, onImport }: Props) {
  ...
  const budget = loadBudgets().find(b => b.month === `${new Date().getFullYear()}-${String(new Date().getMonth() + 1).padStart(2, '0')}`);
  return <div className="content settings-page">
    ...
    <div className="settings-grid">
      <section className="panel settings-card assistant-settings-card"><div className="settings-card-head"><div className="settings-icon assistant-settings-icon"><Bot size={18}/></div><div><h2>Nivora Intelligence</h2><p>Ask questions about your real financial activity.</p></div></div><div className="assistant-settings-preview"><strong>Money, explained.</strong><span>Spending · budget · income · savings</span><button className="secondary-button" onClick={() => setAssistantOpen(true)}>Open assistant <Bot size={16}/></button></div></section>
